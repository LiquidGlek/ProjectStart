...
Every task reads `AGENTS.md`; the outcome and user-intent sections of `PROJECT_CHARTER.md`; the Primary Outcome Lock and assigned rows in `MASTER_CHECKLIST.md`; the task registry/ownership rows in `COORDINATION_BOARD.md`; and its own checklist. The Project Director also reads `PRIOR_ART_RESEARCH.md`, `PROJECT_PLAN.md`, and `TEAM_OPERATING_MODEL.md` completely. Read other controls only when they apply: `VISUAL_PROTOCOL.md` for reference-driven work, `RUNTIME_OWNERSHIP.md` before shared runtime mutation, `INTEGRATION_CONTRACTS.md` for shared seams/workspaces, `RESOURCE_BUDGET.md` for retries/processes or user interference, `TIMEBOX_PROTOCOL.md` at deadline checkpoints, `FOCUS_PROTOCOL.md` on drift/test decisions, `AGENT_COMMUNICATION.md` for cross-task routing, and evidence/decision ledgers when writing or reviewing those records.
...
- At the start of every user turn, classify new content as a requirement, correction, decision, acceptance evidence, or ordinary discussion and persist durable items before resuming affected work.
...
- `spawn_agent` is allowed only for one-shot temporary support inside an existing top-level task when it owns no project lane, production target, master ID, checklist, deadline, runtime, or independent approval. If uncertain, create a top-level task.
