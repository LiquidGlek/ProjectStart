...
0. The Director must already have the user's exact 1–15 non-Director worker count and worker model/effort policy recorded with a timestamped intake receipt. If either is absent, stop before initialization/planning/task creation and ask the two questions together. Do not infer or substitute either answer.
...
- A created or resumed task performs startup verification first and reports exact ID/deeplink, actual model/effort, actual project/root/worktree, checklist, and status. Its actual model/effort must match the assignment derived from the user's recorded model policy. Intended launch settings are not proof; a mismatch stops substantive work and requires safe archive/replacement without silent model substitution.
...
- The answered worker count is binding: exactly 1 in `SOLO`, the answered 2–9 in `SMALL TEAM`, or the answered 10–15 in `FULL TEAM`. Requested, planned, launched, and active counts match during BUILD. In FULL TEAM, `IMPLEMENTATION` is a strict majority. Freeze contracts and use fixtures/test doubles, owned-side adapters, and isolated worktrees so ordinary sibling implementation does not collapse the wave to one or two tasks. Duplicate audits, tiny tests, and status-only tasks do not count.
- At the FULL TEAM regroup barrier, every planned worker returns a coherent handoff or explicit accepted blocker disposition. Freeze new feature work, integrate in the declared batch/order, route conflicts and QA fixes to original stable lane tasks, and create one exact candidate before independent QA.
